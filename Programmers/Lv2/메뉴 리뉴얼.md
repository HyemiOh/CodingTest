...
class Solution {
    ...
    fun solution(orders: Array<String>, course: IntArray): Array<String> {
        ...
        val groupOrders = info.groupBy { it }.filter { it.value.size > 1 }

        course.forEach { num ->
            val filter = groupOrders.filter { it.key.length == num }
            if(filter.isNotEmpty()) {
                val max = filter.maxOf { it.value.size }
                answer.addAll(filter.filter { it.value.size == max }.keys)
            }
        }
...
```
- 조합(순서에 관계없이 중복을 허용하지 않는 경우의 수)으로 손님들이 주문한 단품메뉴 2개 이상의 조합을 추출해낸다.
- 2번 이상 주문된 조합중 course별로 제일 많이 주문된 조합을 오름차순으로 sort하여 리턴해준다.