    ...
    fun solution(want: Array<String>, number: IntArray, discount: Array<String>): Int {
        var answer: Int = 0
		    ...
		    for (i in 0..discount.size - number.sum()) {
		        val sale = discount.slice(i until i + number.sum())
		        var isDiscount = true
		        want.forEachIndexed { index, s ->
		            if(sale.count { it == s } != number[index]) {
		                isDiscount = false
...
- discount 첫날부터 정현이가 원하는 제품의 개수만큼 array를 잘라서 그 리스트 안에 정현이가 원하는 물품과 개수가 있는지 체크