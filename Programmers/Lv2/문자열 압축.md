...
[코딩테스트 연습 - 문자열 압축](https://school.programmers.co.kr/learn/courses/30/lessons/60057)
    ...
    fun solution(s: String): Int {
        ...
        for (i in 1..s.length / 2) {
            ...
            for(j in i .. s.length - i step i){
                rest = s.length - j - i
                val sub = s.substring(j, j + i)
                if (prev == sub) {
                    count++
                } else {
                    if(count > 1){
                        compressed += count.toString().length
                    }
                    count = 1
                    compressed += i
            ...
            answer = minOf(answer,compressed + rest)
        }
        return answer
    }
}
