...
```kotlin
class Solution {
    fun solution(wallpaper: Array<String>): IntArray {val answer = IntArray(4)
        val file = '#'

        answer[0] = (wallpaper.indexOfFirst { it.contains(file) })
        answer[2] = (wallpaper.indexOfLast { it.contains(file) } + 1)
        
        val map = wallpaper.filter { it.contains(file) }.map { wall ->
            Pair(wall.indexOfFirst { it == file}, wall.indexOfLast { it == file })
        }
        answer[1] = (map.minOf { it.first })
        answer[3] = (map.maxOf { it.second } + 1)
